(function () {
  ...
  const context = window.ElyhaWebAppContext || {};
  const {
    constants,
    helpers,
    components,
  ...
  function App() {
    ...
    const [inspectorPreviewPanel, setInspectorPreviewPanel] = useState("outline");
    ...
    useEffect(
      function () {
        setInspectorPreviewPanel("outline");
      },
      [selectedNodeId]
    );
    ...
    function setInspectorMetadataField(field, value) {
      setInspector(function (prev) {
        if (!prev) {
          return prev;
        }
        let metadata = {};
        const source = String(prev.metadata_json || "").trim();
        if (source) {
          try {
        ...
        return Object.assign({}, prev, { metadata_json: JSON.stringify(next, null, 2) });
      });
    }
        ...
        h(
          ...
          h(
            ...
            h(
              "button",
              {
                className: "ghost-node-btn lock" + (locked ? " selected" : ""),
                onClick: function (event) {
                  event.stopPropagation();
                  toggleGhostLock(plan.id);
                }
              },
              locked ? t("web.ghost.unlock") : t("web.ghost.lock")
            ),
            h(
              "button",
              {
    ...
    const activeChatContextId = String(chatContextNodeId || artifactContextNodeId || "").trim();
    const scopedChatMessages = chatMessages.filter(function (item) {
      return String(item && item.context_node_id ? item.context_node_id : "").trim() === activeChatContextId;
    });
            ...
            h(
              ...
              inspector
                ? h(
                    ...
                    (function () {
                      let parsedMetadata = {};
                      const source = String(inspector.metadata_json || "").trim();
                      if (source) {
                        try {
                          const parsed = JSON.parse(source);
                          if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
                      ...
                      const outlineText = parsedMetadata.outline_markdown == null
                        ? ""
                        : String(parsedMetadata.outline_markdown);
                      const contentText = parsedMetadata.content == null ? "" : String(parsedMetadata.content);
                      const hasOutline = Boolean(outlineText.trim());
                      const hasContent = Boolean(contentText.trim());
                      let activePanel = inspectorPreviewPanel;
                      if (activePanel === "outline" && !hasOutline && hasContent) {
                        activePanel = "content";
                      }
                      if (activePanel === "content" && !hasContent && hasOutline) {
                        activePanel = "outline";
                      }
                      ...
                      return h(
                        "div",
                        { className: "field-label" },
                        h("span", null, t("web.inspector.preview_label")),
                        h(
                          "details",
                          {
                            className: "inspector-preview-fold",
                          ...
                          h(
                            "summary",
                            { className: "inspector-preview-head" },
                            h("span", null, t("web.inspector.file_outline")),
                            h("span", { className: "muted" }, hasOutline ? "" : "-")
                          ),
                          h(
                            "div",
                            { className: "inspector-preview-body" },
                            h("textarea", {
                              rows: 6,
                              value: outlineText,
                              placeholder: "-",
                              readOnly: false,
                              onChange: function (event) {
                                setInspectorMetadataField("outline_markdown", event.target.value);
                              }
                            })
                          )
                        ),
                        h(
                          "details",
                          {
                            ...
                            onToggle: function (event) {
                              if (event.target && event.target.open) {
                                setInspectorPreviewPanel("content");
                              }
                            }
                          },
                          h(
                            "summary",
                            { className: "inspector-preview-head" },
                            h("span", null, t("web.inspector.file_content")),
                            h("span", { className: "muted" }, hasContent ? "" : "-")
                          ),
                          h(
                            "div",
                            { className: "inspector-preview-body" },
                            h("textarea", {
                              ...
                              readOnly: false,
                              onChange: function (event) {
                                setInspectorMetadataField("content", event.target.value);
                              }
                            })
                          )
                        )
                      ...
                      h(
                        ...
                        h(
                          ...
                          h(
                            ...
                            t("web.chat.clear_current_history")
                          ),
                          h(
                            "div",
  ...
  window.ElyhaWebAppComponent = App;
})();
